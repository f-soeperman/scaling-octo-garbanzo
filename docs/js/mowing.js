...
async function loadData() {
  ...
  try {
    ...
    // overgangs-terugval.
    state.data = await artefactReadJSON("mowing_data.json", "mowing_data.json");
  ...
  document.getElementById("content").innerHTML = `
    ...
    <div class="grid">
      <div class="specimen-card">
        ...
        <p id="chart-note" style="margin-top:10px;font-size:12px;color:var(--ink-soft);font-style:italic;"></p>
      </div>
    </div>
...
// De curve heeft twee vlakke stukken die er als tekenfout uitzien maar het
// geen van beide zijn. `chartNote` schrijft eronder wélk van de twee je ziet,
// zodat de vraag "waarom loopt dit plat?" niet elke maand terugkomt.
//
//  · vandaag  — de dag is nog niet om. De bodem-check schaalt today's ET0 met
//    de fractie zonnestraling die al gevallen is, dus de groei-eenheid telt
//    alleen de groei *tot nu*. Op een ochtendrun is dat bijna nul; de punt
//    loopt in de loop van de dag vanzelf vol (de workflow draait uurlijks).
//  · staart   — de voorspelling rekent zónder beregening. Loopt de bodem-
//    uitputting op boven DROUGHT_DEPLETION_PCT, dan knijpt de FAO-56 Ks-factor
//    de transpiratie dicht en stopt de accumulatie. Dat is de modeluitspraak
//    "zonder water stopt de groei" — precies het signaal om wél te sproeien,
//    dus die vlakte hoort er te staan.
function chartNote(series, droughtPct) {
  const dry = series.filter(s => s.forecast && s.depletion != null
    && s.depletion >= droughtPct).length;
  const parts = [];
  if (series.some(s => s.partial))
    parts.push("Vandaag telt alleen de groei tot nu — die punt loopt in de loop van de dag vol.");
  if (dry >= 2)
    parts.push("De voorspelling rekent zónder beregening: de bodem droogt uit, "
      + "waardoor de groei achterin de curve stilvalt. Geef water en de lijn loopt door.");
  return parts.join(" ");
}
...
function drawChart() {
  ...
  // Oudere artefacten dragen DROUGHT_DEPLETION_PCT niet — dan de ingebouwde waarde.
  const droughtPct = d.params.DROUGHT_DEPLETION_PCT ?? 55;
  document.getElementById("chart-note").textContent = chartNote(series, droughtPct);
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            label: c => {
              ...
              if (s.depletion != null) lines.push(`Bodemuitputting: ${s.depletion.toFixed(0)}%`);
              // De twee vlakke stukken benoemen zichzelf, zie chartNote().
              if (s.partial) lines.push("⏳ dag nog niet om — groei tot nu");
              else if (s.forecast && s.depletion != null && s.depletion >= droughtPct)
                lines.push("🥵 droogtestress remt de groei");
              return lines;
            },
          },
        },
...
async function saveMow() {
  ...
  try {
    ...
    // Bewust géén workflow_dispatch meer (publiek run-event op de minuut van de
    // registratie); de uurlijkse run pikt het logboek vanzelf op.
  } catch (e) {
