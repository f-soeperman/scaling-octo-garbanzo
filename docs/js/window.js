...
async function loadData() {
  ...
  // Zonder token+artefact-gist valt er niets te tonen — het artefact draagt per
  // kamer 48u temperatuur én vocht op kwartierresolutie, en dat leest als het
  // dagritme van het huishouden (privacy-sweep aug 2026). Zelfde poort als vent.js.
  if (!(CONFIG.githubToken && CONFIG.artefactGistId)) {
    document.getElementById("banner-slot").innerHTML =
      ...
      `om het raamadvies en de kamertemperaturen te zien.</div>`;
    document.getElementById("source-label").textContent = "";
    document.getElementById("content").innerHTML = "";
    return;
  }
  try {
    state.data = await artefactReadJSON("window_data.json", "window_data.json");
    render();
  } catch (e) {
    document.getElementById("banner-slot").innerHTML =
...
function drawCharts() {
  ...
  const measRH = hist.map((p, i) => ({ x: histT[i], y: p.rh20 != null ? p.rh20 : null }));
  const usedRH = [...hist.map((p, i) => ({ x: histT[i], y: p.rh20 != null ? p.rh20 : null })),
                  ...fut.map((f, i) => ({ x: futT[i], y: f.rh20_corr != null ? f.rh20_corr : null }))];
...
function drawScatter() {
  ...
  // alleen verder oprekken als een punt erbuiten valt, met wat marge.
  const HUM_MARGIN = 5;
