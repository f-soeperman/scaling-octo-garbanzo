...
// op het bodem/gazon-dashboard). Wijzigen vereist jouw token — die staat enkel lokaal.
// Lezen vereist sinds de privatisering (aug 2026) óók de artefact-gist-koppeling: het
// dashboard toonde tot dan de gerapporteerde raamstanden (en de plattegrond die daaruit
// getekend wordt) aan iedereen — dat is precies wat privé moest worden.
...
async function loadData() {
  ...
  // Zonder token+artefact-gist is er niets te tonen — de raamstanden zijn privé.
  // Anders dan index/mowing.js retryen we hier niet automatisch op een falende fetch
  // (die kans krijgt de bezoeker toch niet zonder token): eerst koppelen, dan pas laden.
  if (!(CONFIG.githubToken && CONFIG.artefactGistId)) {
    document.getElementById("banner-slot").innerHTML =
      `<div class="banner banner-warn">🔒 Dit dashboard is privé — koppel je account (🔑 Account hierboven) ` +
      `om de raamstanden en kamertemperaturen te zien.</div>`;
    document.getElementById("source-label").textContent = "";
    document.getElementById("content").innerHTML = "";
    return;
  }
  try {
    state.data = await artefactReadJSON("vent_data.json", "vent_data.json");
  } catch (e) {
    document.getElementById("banner-slot").innerHTML =
      `<div class="banner banner-error">Kon <code>vent_data.json</code> niet laden (${e.message}). ` +
      `Het model draait elk kwartier via GitHub Actions.</div>`;
    document.getElementById("source-label").textContent = "";
    document.getElementById("content").innerHTML = "";
    return;
  }
  ...
  try {
    state.forecast = await artefactReadJSON("vent_forecast.json", "vent_forecast.json");
  } catch (e) {
    console.warn("vent_forecast.json niet geladen:", e);
    state.forecast = null;
  }
  // Empirische p10/p90-band per (kamer, horizon-uur) — wekelijks ververst door
  // twin-eval.yml. Alleen-verrijking: ontbreekt of faalt hij, dan tekent de
  // temperatuurgrafiek gewoon de kale vooruitbliklijn (zoals vóór aug 2026).
  try {
    const res = await fetch(bust("js/uncertainty.json"));
    state.uncertainty = res.ok ? await res.json() : null;
  } catch (e) {
    console.warn("uncertainty.json niet geladen:", e);
...
function render() {
  ...
  // Pauze/airco staan bewust niet meer in het publieke artefact (huisboekhouding); de
  // modal toont de actuele stand aan token-houders via de live Gist-read.
  document.getElementById("banner-slot").innerHTML = "";

  const rmse = d.learned && d.learned.rmse != null ? d.learned.rmse : null;
  let html = "";

  // — Strip: weer + zon + wind + leer-RMSE (alleen de anomalie-held-banner — de rest van de
...
// p10/p90-band rond de vooruitblik van één kamer. De cellen in uncertainty.json zijn
// (voorspeld − gemeten)-kwantielen per horizon-uur, dus de band op een voorspelling is
// [pred − p90, pred − p10]; tussen de uur-cellen lineair, buiten de gedekte uren geklemd
// op de rand, en alleen trusted cellen tellen. Twee eerlijke grenzen die het bestand
// zelf ook meldt: modelfout only (geen weersvoorspelfout) en gemeten op de standen die
// het huis werkelijk had.
function bandSeries(rid, fc, now) {
  const cells = state.uncertainty && state.uncertainty.bands && state.uncertainty.bands[rid];
  if (!cells || !fc || !fc.length) return null;
  const hours = Object.keys(cells).map(Number)
    .filter(h => cells[String(h)] && cells[String(h)].trusted).sort((a, b) => a - b);
  if (!hours.length) return null;
  const at = h => {
    const hh = Math.min(Math.max(h, hours[0]), hours[hours.length - 1]);
    let lo = hours[0], hi = hours[hours.length - 1];
    for (const x of hours) if (x <= hh) lo = x;
    for (let i = hours.length - 1; i >= 0; i--) if (hours[i] >= hh) hi = hours[i];
    const a = cells[String(lo)], b = cells[String(hi)];
    const w = hi === lo ? 0 : (hh - lo) / (hi - lo);
    return { p10: a.p10 + (b.p10 - a.p10) * w, p90: a.p90 + (b.p90 - a.p90) * w };
  };
  const lo = [], hi = [];
  for (const p of fc) {
    const x = new Date(p.t).getTime();
    const h = (x - now) / HOUR_MS;
    if (h <= 0) continue;
    const cell = at(h);
    lo.push({ x, y: p.temp - cell.p90 });
    hi.push({ x, y: p.temp - cell.p10 });
  }
  return lo.length ? { lo, hi } : null;
}
...
function drawTempChart() {
  ...
  Object.entries(state.data.rooms || {}).forEach(([rid, r]) => {
    ...
    // Onzekerheidsband alleen op de toekomst (het verleden hééft een meting ernaast).
    // "_"-labels worden uit legenda + tooltip gefilterd — het zijn hulpdatasets.
    const band = bandSeries(rid, r.forecast_series, now);
    if (band) {
      ds.push({ label: `_band_lo_${rid}`, data: band.lo, borderWidth: 0, pointRadius: 0,
                tension: 0.25, fill: false, order: 8 });
      ds.push({ label: `_band_hi_${rid}`, data: band.hi, borderWidth: 0, pointRadius: 0,
                tension: 0.25, fill: "-1", backgroundColor: col + "24", order: 8 });
    }
  ...
  state.tempChart = new Chart(c, { type: "line", data: { datasets: ds }, options: {
    ...
    plugins: {
      ...
      tooltip: { ...ventTooltip(), filter: it => !(it.dataset.label || "").startsWith("_") },
...
async function openReport() {
  ...
  // Meldingen-toggle (stille modus) — eigen Gist-bestand, los van de openingen-log;
  // laadt asynchroon zodat een trage read de modal niet ophoudt.
  buildQuietToggle(null);
  fetchNotifyPrefs().then(buildQuietToggle).catch(() => {});
...
// Gist-log — het publieke artefact draagt de pauzestand bewust niet meer, dus zonder
// token(-read) valt de toggle terug op "niet gepauzeerd". Zet ook meteen de grijze-uit-
// stand op #ac-row + #ctl-list als de modal met een al-actieve pauze opent.
...
// ── Stille modus (notify_prefs.json — eigen Gist-bestand, browser = enige schrijver) ──
// De keuze leeft bewust BUITEN state.pending: hij mag nooit als sleutel in de
// openingen-log belanden (die accumuleert voorwaarts en werd vroeger publiek
// gearchiveerd). Schakelt direct bij de klik — geen "Bewaar & leer" nodig, en er
// wordt (net als bij de Gardena-vlag) bewust géén workflow gedispatcht.
const PREFS_FILE = "notify_prefs.json";

async function fetchNotifyPrefs() {
  if (!(CONFIG.githubToken && CONFIG.gistId && CONFIG.gistId !== "__GIST_ID__")) return null;
  try { return (await gistReadJSON(PREFS_FILE, {})) || {}; }
  catch (e) { console.warn("notify_prefs lezen mislukt:", e); return null; }
}

function renderQuietSeg(cur, busy) {
  const seg = document.getElementById("quiet-seg");
  if (!seg) return;
  seg.classList.toggle("disabled", !!busy);
  seg.querySelectorAll("button").forEach(b => {
    b.classList.toggle("active", cur !== null && (b.dataset.v === "true") === cur);
  });
}

function buildQuietToggle(prefs) {
  const seg = document.getElementById("quiet-seg");
  if (!seg) return;
  renderQuietSeg(prefs === null ? null : !!prefs.quiet, false);
  // onclick-toewijzing (geen addEventListener): de modal kan vaker openen en een
  // gestapelde listener zou de Gist dubbel beschrijven.
  seg.onclick = async (e) => {
    const btn = e.target.closest("button[data-v]");
    if (!btn || !ensureToken()) return;
    const v = btn.dataset.v === "true";
    renderQuietSeg(v, true);
    try {
      const cur = (await gistReadJSON(PREFS_FILE, {})) || {};
      if (!!cur.quiet !== v) {
        const now = new Date().toISOString();
        const next = { schema: 1, quiet: v,
                       since: v ? now : (cur.since || null),
                       cleared_at: v ? (cur.cleared_at || null) : now,
                       updated_at: now };
        await gistWriteFile(PREFS_FILE, JSON.stringify(next, null, 2));
      }
      renderQuietSeg(v, false);
    } catch (err) {
      alert("Meldingen-stand opslaan mislukt: " + err.message);
      renderQuietSeg(null, false);
      fetchNotifyPrefs().then(buildQuietToggle).catch(() => {});
    }
  };
}
...
// Vul de airco-dropdown: "geen" + elke sensorkamer (uit house_meta — het publieke artefact
// draagt de airco-boekhouding niet meer; de huidige keuze komt uit de live Gist-log).
// Keuze → `ac_room` ("" = geen) in state.pending.
...
async function saveReport() {
  ...
  try {
    ...
    // Bewust géén workflow_dispatch meer: zo'n run is publiek zichtbaar (actor + minuut)
    // en dateert precies wanneer iemand thuis een stand meldde. De kwartierloop pikt de
    // melding vanzelf op.
    toggleModal(false);
    document.getElementById("banner-slot").innerHTML =
      `<div class="banner banner-ok">Standen bewaard — het model leert er bij de volgende run van (binnen ~15 min).</div>`;
  } catch (e) {
