...
function sandboxCardHTML() {
    ...
    return `<div class="grid" style="grid-template-columns:1fr;"><div class="specimen-card" id="sandbox-card">
...
function floorPlanSVG(d, opts={}) {
  ...
  // het verdieping-label van een rij, zodat de rij op bedroom' hoogte "1e verdieping" toont.
  const floorsByRow = {};
...
function windPressure(facadeAz, height, windSpeed, windDir, shelter, rho, cpFn) {
  ...
  return shelter * (cpFn || cpCoeff)(theta) * 0.5 * rho * u * u;
}
...
function buildOpeningsJS(meta, states, wind, params, outsideTemp) {
  const cd = params.cd ?? 0.62, rhoOut = airDensity(outsideTemp);
