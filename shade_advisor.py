...
def _send(msg: str) -> None:
    # Berichttekst (welke ramen wanneer dicht) alleen onder DRY_RUN printen —
    # hoort niet in het publieke Actions-log.
...
def run_plan(now: datetime) -> None:
    ...
    # Bewust GEEN `windows`-payload in de gecommitte state: die lijst is een afgeleide
    # van de privé openingen-log (welke zonwering vanochtend open gemeld stond) en dit
    # bestand is publiek. De reminder-fase herrekent het plan zelf (run_reminder);
    # de state draagt alleen de timing/dedup-vlaggen die de orchestrator leest.
...
def run_reminder(now: datetime) -> None:
    ...
    # Herreken het plan van vanochtend uit de live bronnen: de state draagt bewust geen
    # `windows`-payload meer (afgeleide van de privé openingen-log in een publiek bestand).
    # Bijkomend voordeel: een zonwering die intussen dicht gemeld is valt er vanzelf uit
    # (ΔW≈0) en de herinnering wijst dan naar het eerstvolgende raam dat er nog toe doet.
    try:
        weather = vio.fetch_weather(lat, lon)
        log = vio.load_openings_log()
        states_now = vio.openings_at(log, now)
        ...
        print(f"[zonwering] herrekenen plan faalde ({sanitize_error(e)}) — uitstel.")
        return
    if not plan["windows"]:
        print("[zonwering] geen actionabel raam meer bij herrekenen — reminder "
              "stilletjes opgegeven.")
