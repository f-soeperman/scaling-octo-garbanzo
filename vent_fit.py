...
def coupled_sensorless_zones(house: dict, rooms: list[str]) -> list[str]:
    ...
    sterk gekoppeld aan office, bedroom en nursery, dus zijn parameters zijn wel degelijk
...
def thin_rmse_history(hist: list[dict], now: datetime) -> list[dict]:
    ...
    for p in hist:
        ...
        if best is None:
            ...
            buckets[key] = p
            continue
        # `held` is sinds de artefact-scrub de enige non-live-marker per punt (anomalie-hold);
        # pauze-holds worden bewust niet meer per punt gepubliceerd of bewaard.
        cand_live = not p.get("held")
        best_live = not best.get("held")
