...
def _heat_house():
    return {
        ...
        "rooms": {"nursery": {"from_window_data": "Nursery", "volume_m3": 30, "exterior_wall_m2": 10,
                          "plan_xy": [0, 0]},
...
def test_collect_heating_on_en_filter():
    ...
    actual = {"nursery": [(t, 0.0) for t in ts], "bath": [(t, 0.0) for t in ts]}
    filt, excl = vf.filter_heating_samples(actual, heat_on)
    assert excl == {"nursery": 2}                           # twee stook-samples weg
    assert len(filt["nursery"]) == 3 and len(filt["bath"]) == 5
    assert all(t not in heat_on["nursery"] for t, _ in filt["nursery"])
...
def test_filter_heating_drops_room_when_all_heated():
    ...
    actual = {"nursery": [(t, 21.0) for t in ts]}
    heat_on = {"nursery": set(ts)}                          # heel het venster gestookt
    filt, excl = vf.filter_heating_samples(actual, heat_on)
    assert "nursery" not in filt                            # kamer valt uit de kalibratie
    assert excl["nursery"] == 4
...
def test_filter_excluded_rooms_drops_the_room_wholesale():
    ...
    actual = {"nursery": [(t0, 22.0), (t0, 22.5)], "bath": [(t0, 25.0), (t0, 31.0), (t0, 26.0)]}
    filt, excl = vf.filter_excluded_rooms(actual, house)
    assert list(filt) == ["nursery"]                # bath valt volledig weg, niet per sample
    assert excl == {"bath": 3}                  # zelfde rapportagevorm als de andere filters
    assert filt["nursery"] == actual["nursery"]         # de rest blijft ongemoeid
...
def test_thin_rmse_history_prefers_live_representative():
    ...
    # `held` (anomalie) is sinds de privacy-scrub de enige non-live-marker per punt.
    hist = [_lp(h0, rmse=0.4),                                        # live, eerst
            _lp(h0 + timedelta(minutes=15), rmse=0.6, held=True),     # later maar held
            _lp(h0 + timedelta(minutes=30), rmse=0.9, held=True)]     # laatst maar held
    out = vf.thin_rmse_history(hist, now)
    assert len(out) == 1 and out[0]["rmse"] == 0.4    # live verdringt latere held-punten
