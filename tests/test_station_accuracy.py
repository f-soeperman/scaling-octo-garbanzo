...
def test_neighbour_coherence_lekt_geen_station_ids(monkeypatch):
    ...
    # Fixture-id's bewust in een vorm die de station-id-regex van de privacy-
    # suite (I[A-Z]{4,}\d+) niet matcht — drie letters volstaan voor de test.
    monkeypatch.setenv("WU_NEIGHBOUR_IDS", "IGEH1,IGEH2")
    monkeypatch.setenv("WU_STATION_ID", "IGEHONS")
    frame = _knmi_frame()
    monkeypatch.setattr(sa, "fetch_wu_hourly", lambda sid, key, days: {
        t: {"temp": v["temp"] + 0.5, "solar": v["solar"], "wind": v["wind"]}
        for t, v in frame.items()})
    out = sa.neighbour_coherence("key", "2026-08-01", "2026-08-10", 10, frame)
    assert "IGEH" not in json.dumps(out)
