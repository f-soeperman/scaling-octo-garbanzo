...
# ── Stille modus (quiet gate) ──────────────────────────────────────────────────

@pytest.fixture
def quiet_env(monkeypatch):
    """Creds gezet, prefs-cache schoon; `posts` vangt elke echte Telegram-POST."""
    posts = []
    ...
    notify._reset_prefs_cache()
    yield posts
    notify._reset_prefs_cache()


def _set_prefs(monkeypatch, prefs):
    monkeypatch.setattr(notify, "_fetch_notify_prefs", lambda: prefs)
    notify._reset_prefs_cache()


def test_quiet_muted_send_gedraagt_zich_als_verzonden(quiet_env, monkeypatch, capsys):
    """De kern van de stille modus: geen POST, maar exact dezelfde stdout en dezelfde
    return-waarde als een geslaagde verzending — aanroepers stempelen hun meld-state
    dus identiek en er is geen publiek waarneembaar verschil."""
    posts = quiet_env
    _set_prefs(monkeypatch, {"quiet": True, "since": "2026-08-01T09:00:00+02:00"})
    assert notify.send_telegram("advies", muted_in_quiet=True) is True
    assert posts == []
    stil = capsys.readouterr().out

    _set_prefs(monkeypatch, {"quiet": False})
    assert notify.send_telegram("advies", muted_in_quiet=True) is True
    assert len(posts) == 1
    normaal = capsys.readouterr().out
    assert stil == normaal == "[telegram] ✓ verzonden\n"


def test_quiet_raakt_ongemuted_berichten_niet(quiet_env, monkeypatch):
    """Operationele meldingen (crash-alerts, token-persist, Gardena) geven de vlag niet
    mee en gaan ook in stille modus gewoon uit — én ze lezen de prefs niet eens."""
    posts = quiet_env

    def boom():
        raise AssertionError("ongemuted bericht mag de prefs niet lezen")

    monkeypatch.setattr(notify, "_fetch_notify_prefs", boom)
    notify._reset_prefs_cache()
    assert notify.send_telegram("ops-alert") is True
    assert len(posts) == 1


def test_quiet_geen_creds_pad_identiek(monkeypatch, capsys):
    """De poort staat ná de creds-check: zonder creds is het gedrag in beide standen
    exact hetzelfde (False + dezelfde regel)."""
    monkeypatch.delenv("TELEGRAM_BOT_TOKEN", raising=False)
    monkeypatch.delenv("TELEGRAM_CHAT_ID", raising=False)
    _set_prefs(monkeypatch, {"quiet": True})
    assert notify.send_telegram("x", muted_in_quiet=True) is False
    assert capsys.readouterr().out == "[telegram] geen creds, overslaan\n"


def test_quiet_prefs_gecachet_per_proces(quiet_env, monkeypatch):
    reads = []
    monkeypatch.setattr(notify, "_fetch_notify_prefs",
                        lambda: reads.append(1) or {"quiet": True})
    notify._reset_prefs_cache()
    notify.send_telegram("a", muted_in_quiet=True)
    notify.send_telegram("b", muted_in_quiet=True)
    assert len(reads) == 1


def test_quiet_fail_open_zonder_gist_creds(quiet_env, monkeypatch):
    """Geen GIST_ID in de omgeving → niet stil (fail-open): het bericht gaat gewoon uit."""
    posts = quiet_env
