...
def _write_state(path, **over):
    # De gecommitte state draagt sinds de privacy-scrub alleen nog timing/dedup-vlaggen —
    # géén `windows`-payload (afgeleide van de privé openingen-log); de reminder herrekent
    # het plan zelf uit de live bronnen.
...
def _weather(now, current):
    return {"hourly": _sunny_rows(now), "current": current}
...
def test_reminder_materialization_hold_then_send(state_env, monkeypatch):
    ...
    monkeypatch.setattr(vio, "fetch_weather", lambda lat, lon: _weather(
        NOW, {"direct_radiation": 0.0, "shortwave_radiation": 10.0}))
    sa.run_reminder(NOON)
    assert sent == [] and not sa.load_state()["reminder_sent"]
    # Zon breekt door: voorspelde last materialiseert → één bericht + sent-vlag.
    monkeypatch.setattr(vio, "fetch_weather", lambda lat, lon: _weather(
        NOW, {"direct_radiation": 600.0, "shortwave_radiation": 800.0}))
    sa.run_reminder(NOON)
...
def test_reminder_geeft_op_als_herrekend_plan_leeg_is(state_env, monkeypatch):
    """De bewoner heeft de zonwering intussen zelf dichtgemeld → het herrekende plan
    heeft geen actionabel raam meer en de reminder geeft stilletjes op (dedup-vlag
    gezet, geen bericht). Dit is het nieuwe pad dat de `windows`-payload vervangt."""
    path, sent = state_env
    _write_state(path)
    monkeypatch.setattr(vio, "load_house", lambda: HOUSE)
    monkeypatch.setattr(vio, "load_openings_log", lambda: [])
    monkeypatch.setattr(vio, "openings_at",
                        lambda log, now: {"sky_shade": "dicht", "lam_shade": "dicht"})
    monkeypatch.setattr(vio, "fetch_weather", lambda lat, lon: _weather(
        NOW, {"direct_radiation": 600.0, "shortwave_radiation": 800.0}))
    sa.run_reminder(NOON)
...
def test_reminder_gives_up_after_window(state_env, monkeypatch):
    """Voorbij het open-moment van het (herrekende) dicht-venster is herinneren
    zinloos → stil opgeven."""
    path, sent = state_env
    _write_state(path)
    laat = NOW.replace(hour=21, minute=30)               # avond: span allang voorbij
    monkeypatch.setattr(vio, "load_house", lambda: HOUSE)
    monkeypatch.setattr(vio, "load_openings_log", lambda: [])
    monkeypatch.setattr(vio, "fetch_weather", lambda lat, lon: _weather(
        NOW, {"direct_radiation": 0.0, "shortwave_radiation": 0.0}))
    sa.run_reminder(laat)
    assert sent == [] and sa.load_state()["reminder_sent"]
...
def test_run_plan_state_draagt_geen_raamstand_afgeleiden(state_env, monkeypatch):
    """shade_state.json is publiek gecommit; het mag alléén timing/dedup-vlaggen dragen.
    De oude `windows`-payload codeerde welke zonwering die ochtend open gemeld stond —
    een afgeleide van de privé openingen-log (privacy-scrub, aug 2026)."""
    path, _sent = state_env
    monkeypatch.setattr(vio, "load_house", lambda: HOUSE)
    monkeypatch.setattr(vio, "load_openings_log", lambda: [])
    monkeypatch.setattr(vio, "fetch_weather", lambda lat, lon: _weather(NOW, {}))
    sa.run_plan(NOW)
    state = json.loads(path.read_text())
    assert set(state) == {"date", "plan_sent", "reminder_at", "reminder_sent"}
