...
def setup():
    ...
    wd = {"rooms": {"Nursery": {"inside": 22.4, "humidity": 55}}}
    actual = {"nursery": [(NOW - timedelta(hours=1), 22.0), (NOW, 22.4)]}
...
def test_dashboard_schema_toplevel(dash):
    verwacht = {"generated_at", "as_of_local", "source", "model_version", "weather",
                "openings", "controls", "rooms", "flows", "learned", "house_meta"}
    ...
    # Privacy-scrub: pauze- en airco-boekhouding staan bewust NIET in het publieke
    # artefact (de modal leest ze live uit de Gist). Dit is een verboden-velden-pin.
    assert not {"paused", "paused_since", "ac"} & set(dash)
    assert dash["openings"] == {}                     # lege log → geen gemelde standen
    assert isinstance(dash["controls"], list) and dash["controls"]
    # De airco-dropdown leunt op de sensor-vlag in house_meta (géén apart ac-blok meer).
    sensor_ids = {rid for rid, r in dash["house_meta"]["rooms"].items() if r["sensor"]}
    assert sensor_ids == {"living", "nursery", "bedroom", "office", "bath"}   # géén stair/hall
...
def test_dashboard_schema_kamers(setup, dash):
    room_keys = {"label", "actual_temp", "predicted_temp", "predicted_air_temp",
                 ...
                 "trend_c_per_h", "comfort_low", "comfort_high", "heating",
                 "fit_excluded", "hidden", "predicted_series",
                 "forecast_series", "actual_series", "params"}
    # `ac`/`paused` zijn privacy-gescrubde velden (huisboekhouding hoort niet in een
    # publiek artefact); de rest is de oude schema-sanering.
    geschrapt = {"from_window_data", "party_w", "ground_w", "internal_w",
                 "ac_excluded_samples", "ac", "paused"}
    ...
    for rid, room in dash["rooms"].items():
        ...
        assert isinstance(room["heating"], bool)
    ...
    assert dash["rooms"]["nursery"]["heating"] is True
    assert dash["rooms"]["living"]["heating"] is False
    # Nursery heeft wd-data → actual_temp/humidity/error gevuld; comfortband uit ROOM_COMFORT.
    nursery = dash["rooms"]["nursery"]
    assert nursery["actual_temp"] == pytest.approx(22.4)
    assert nursery["humidity"] == 55
    assert _num(nursery["error"])
    assert nursery["comfort_low"] == pytest.approx(17.0)
    assert nursery["comfort_high"] == pytest.approx(18.0)
    assert len(nursery["actual_series"]) == 2
...
def test_dashboard_stempelt_de_structurele_uitsluitingen(dash):
    ...
    for rid in ("living", "nursery", "bedroom", "office"):
    ...
    assert dash["house_meta"]["rooms"]["bath"]["sensor"] is True   # blijft in de ac-dropdown
...
def test_dashboard_draagt_de_vooruitblik_door(setup):
    ...
    fc = vfc.forecast(house, setup["params"], tl, sim12, NOW, setup["ctx"],
                      {"nursery": sim12["Ta_now"]["nursery"] + 1.0})
    d = _dash(dict(setup, timeline=tl, sim=sim12), forecast=fc)
    nursery = d["rooms"]["nursery"]
    assert nursery["forecast_series"], "geen vooruitblik doorgegeven"
    assert all(datetime.fromisoformat(pt["t"]) >= NOW for pt in nursery["forecast_series"])
    span_h = (datetime.fromisoformat(nursery["forecast_series"][-1]["t"]) - NOW).total_seconds() / 3600
    ...
    assert leeg["rooms"]["nursery"]["forecast_series"] == []
...
def test_dashboard_schema_learned_blok(dash):
    ...
    assert "paused" not in ln                          # privacy-scrub: geen pauze-marker
...
def test_rmse_history_groeit_met_precies_een_punt(setup):
    ...
    assert {"t", "rmse", "held", "version"} <= set(nieuw)
    assert set(nieuw) <= {"t", "rmse", "held", "version",
                          "skill", "rmse_naive", "wx"}
    assert nieuw["t"] == NOW.isoformat()
    assert nieuw["rmse"] == pytest.approx(0.7)
    assert nieuw["held"] is False
...
def test_oude_punten_worden_van_pauze_marker_ontdaan(setup):
    """Historische leercurve-punten van vóór de scrub dragen `paused` — bij de eerstvolgende
    schrijf wordt dat veld gestript. Een reeks pauze-punten in een publiek artefact is een
    machine-leesbare afwezigheidskalender; dit is de eenmalige opruiming."""
    oud = [{"t": (NOW - timedelta(hours=2)).isoformat(), "rmse": 0.5,
            "held": False, "paused": True, "version": "abc1234"},
           {"t": (NOW - timedelta(hours=1)).isoformat(), "rmse": 0.6,
            "held": True, "paused": False, "version": "abc1234"}]
    d = _dash(setup, learned={"rmse_history": oud}, rmse_now=0.7)
    hist = d["learned"]["rmse_history"]
    assert all("paused" not in p for p in hist)
    assert hist[0]["rmse"] == 0.5 and hist[1]["held"] is True   # rest onaangeroerd


def test_paused_run_publiceert_geen_marker(setup):
    """Een gepauzeerd (of stil-gehouden) venster mag NIET publiek herkenbaar zijn: main()
    geeft alleen de anomalie-hold door als `learning_held`, dus een pauze-run schrijft een
    gewoon punt zonder held/paused-stempel en geen enkel pauze-veld op welk niveau ook."""
    d = _dash(setup, rmse_now=0.9, learning_held=False)
    assert not {"paused", "paused_since", "ac"} & set(d)
    assert "paused" not in d["learned"]
    assert all("paused" not in r and "ac" not in r for r in d["rooms"].values())
    punt = d["learned"]["rmse_history"][-1]
    assert "paused" not in punt and punt["held"] is False


def test_anomalie_hold_stempelt_held(setup):
    """De anomalie-hold blijft wél publiek (zelf-oplossend binnen 24u, dus geen
    afwezigheidssignaal): learned.held en het leercurve-punt dragen de vlag."""
    d = _dash(setup, rmse_now=0.9, learning_held=True)
    assert d["learned"]["held"] is True
    assert d["learned"]["rmse_history"][-1]["held"] is True


def test_openings_filtert_boekhoud_en_dode_sleutels(setup):
    """De openingen-log accumuleert voorwaarts: boekhoud-sleutels (paused/ac_room) en
    hernoemde element-ids blijven er eeuwig in staan. Het publieke artefact filtert op de
    element-ids uit het huismodel — de verboden-sleutels-poort van de privacy-scrub."""
    log = [{"t": (NOW - timedelta(hours=3)).isoformat(),
            "states": {"paused": True, "ac_room": "office",
                       "ted_stair": "open", "kitchen_mid_window": "dicht",
                       "nursery_small_window": "open", "office_vent1": "dicht"}}]
    d = _dash(setup, log=log)
    assert set(d["openings"]) == {"nursery_small_window", "office_vent1"}
    assert d["openings"]["nursery_small_window"] == "open"
    ctl_ids = {c["id"] for c in d["controls"]}
    assert not {"paused", "ac_room", "ted_stair", "kitchen_mid_window"} & ctl_ids
    # De gemelde stand komt wél gewoon in de control-lijst terecht.
    by_id = {c["id"]: c for c in d["controls"]}
    assert by_id["nursery_small_window"]["state"] == "open"
...
def test_controls_filtert_vast_glas_en_vaste_deuren(setup):
    ...
    assert "nursery_window" not in per_kind["window"]
    assert "nursery_small_window" in per_kind["window"]
    ...
    assert by_id["nursery_small_window"]["state"] == "dicht"
...
def test_controls_zonwering_met_suffix(setup):
    ...
    assert shades["nursery_window_shade"]["state"] == "open"
