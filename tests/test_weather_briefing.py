...
UV-wolkencorrectie (Josefsson & Landelius CMF), uv_windows-interpolatie en de
WU-stralingsbiascorrectie. Geen netwerk. De oude thuis/vakantie-detectie is
bewust verwijderd (privacy: een locatiewissel was een publieke commit + logregel);
een test bewaakt dat die niet terugkeert.
"""
...
import wu_bias
from weather_briefing import cloud_corrected_uv, hours_in_window, uv_windows
...
# ── geen locatie-/afwezigheidsmechanisme ───────────────────────────────────────

def test_geen_vakantiemodus_meer():
    """De hand-aanpasbare locatie + thuisdetectie zijn verwijderd: een locatiewissel
    betekende een publieke commit met de bestemming en een afwijkende kop in het
    publieke run-log. De briefing kent alleen nog de vaste thuislocatie; tijdelijk
    stil = de stille modus (notify_prefs), die niets publieks achterlaat."""
    for naam in ("is_home", "haversine_km", "HOME_COORDS", "HOME_RADIUS_KM"):
        assert not hasattr(wb, naam), naam
    assert wb.LOCATION["label"] == "Utrecht"
