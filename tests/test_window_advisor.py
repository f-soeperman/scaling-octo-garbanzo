...
# ── Meldlaag: cooldown, meldgeheugen, duur-poort en de urgente uitzondering ───
...
def _decide_open(state, room="office", intervals=_DEFAULT, now=NOW, **kw):
    ...
    # Zelfde volgorde als main(): het advies-geheugen wordt bijgewerkt vóór de meldpoort,
    # zodat de versheidspoort (OPEN_MSG_MAX_AGE_H) hier hetzelfde ziet als in productie.
    wa.track_open_since(state, room, "open", now)
...
def _decide_dicht(state, room="office", now=NOW, **kw):
    ...
    kw.setdefault("vent_rh", 50.0)
    wa.track_open_since(state, room, "dicht", now)
...
def test_meldlaag_staat_een_open_en_een_dicht_toe():
    state = {}
...
def test_cooldown_blokkeert_een_snelle_heropening():
    """Flapping-bescherming: open → dicht → open binnen de cooldown blijft stil."""
    state = {}
    d = _decide_open(state)
    wa.record_notification(state, "office", d, NOW)
    dd = _decide_dicht(state)
    wa.record_notification(state, "office", dd, NOW)
    binnen = NOW + timedelta(hours=wa.OPEN_MSG_COOLDOWN_H - 0.5)
    assert _decide_open(state, now=binnen) is None


def test_cooldown_loopt_af_en_geeft_weer_ruimte():
    state = {}
    d = _decide_open(state)
    wa.record_notification(state, "office", d, NOW)
    dd = _decide_dicht(state)
    wa.record_notification(state, "office", dd, NOW)
    erna = NOW + timedelta(hours=wa.OPEN_MSG_COOLDOWN_H + 0.1)
    assert _decide_open(state, now=erna) is not None


def test_cooldown_is_per_kamer_niet_per_huis():
    state = {}
    d = _decide_open(state, room="office")
    wa.record_notification(state, "office", d, NOW)
    assert _decide_open(state, room="Nursery") is not None
...
def test_urgent_breekt_door_de_cooldown():
    state = {}
    d = _decide_open(state)
    wa.record_notification(state, "office", d, NOW)
    # De gewone sluitweg dichtzetten met een sluiting, zodat de cooldown nog loopt...
    dd = _decide_dicht(state)
...
def test_urgent_respecteert_cooldown_en_dagmaximum():
    ...
    # kalenderdag blijven, anders reset het urgentie-dagmaximum en meet de test iets anders.
    start = datetime(2026, 8, 1, 7, 0)
    # `msg_at` zet de gewone sluitweg dicht (cooldown loopt nog), zodat alleen de urgente
    # uitzondering er nog doorheen kan — dát is wat deze test meet.
    state = {"notified": {"office": {"state": "open", "at": start.isoformat()}},
             "msg_at": {"office": {"dicht": start.isoformat()}},
             "day": {"date": start.date().isoformat(), "plan_sent": True,
                     "rooms": {"office": {"urgent": 0, "last_urgent": None}}}}
    heet = dict(inside=22.0, outside=22.0 + wa.URGENT_HEAT_C, vent_rh=50.0)
    def _herstel(t):
        # Terug naar "wij hebben open gemeld" én de gewone sluitweg opnieuw dichtzetten:
        # de urgentie-cooldown is langer dan één stap, dus zonder dit verse stempel loopt
        # de normale dicht-cooldown ondertussen af en meet de test die i.p.v. de urgente.
        state["notified"]["office"] = {"state": "open", "at": t.isoformat()}
        state["msg_at"]["office"]["dicht"] = t.isoformat()
...
def test_avondopen_schuift_niet_naar_middernacht():
    """De regressie van 11–12 aug 2026 (Nursery + bedroom).

    `decide()` zei om 20:15 al open, maar het bericht kwam pas om 00:00 — en verbruikte
    daar het open-budget van de níeuwe kalenderdag. Daardoor werd de volgende avond weer
    onderdrukt en schoof het bericht opnieuw naar 00:00: eenmaal over middernacht kwam het
    er nooit meer vanaf, elke nacht goed voor bijna vier uur verloren koelte.

    De cyclus die dat blootlegde: 's avonds open, de volgende ochtend dicht, diezelfde
    avond weer open. Met een dagbudget van 1 open valt die laatste open op dezelfde
    kalenderdag als de ochtendsluiting en wordt hij gesmoord; met een cooldown telt alleen
    de afstand tot het vórige open-bericht (hier ~24u).
    """
    state = {}
    avond = datetime(2026, 8, 11, 20, 15)
    d = _decide_open(state, now=avond)
    assert d is not None, "een verse open-flip hoort meteen gemeld te worden"
    wa.record_notification(state, "office", d, avond)

    ochtend = datetime(2026, 8, 12, 12, 30)
    dd = _decide_dicht(state, now=ochtend)
    assert dd is not None and dd["urgent"] is False
    wa.record_notification(state, "office", dd, ochtend)

    avond2 = datetime(2026, 8, 12, 20, 15)
    d2 = _decide_open(state, now=avond2)
    assert d2 is not None and d2["kind"] == "open", \
        "de tweede avond valt op dezelfde kalenderdag als de ochtendsluiting"


def test_verlopen_open_wens_wordt_niet_alsnog_gemeld():
    """Een onderdrukte open blijft niet hangen om uren later alsnog te vuren.

    Zonder deze poort landde het bericht op het eerste moment dat een andere poort het
    tóéliet — in de praktijk precies op de dagwissel, wanneer niemand er iets mee kan.
    """
    state = {}
    flip = datetime(2026, 8, 11, 20, 15)
    wa.track_open_since(state, "office", "open", flip)
    laat = flip + timedelta(hours=wa.OPEN_MSG_MAX_AGE_H + 0.5)
    assert wa.notify_decision(state, "office", "open", LANG, "cool", inside=22.0,
                              outside=19.0, vent_rh=50.0, now=laat) is None
    # ...maar dezelfde wens is op de flip zelf wél een bericht waard.
    assert wa.notify_decision(state, "office", "open", LANG, "cool", inside=22.0,
                              outside=19.0, vent_rh=50.0, now=flip) is not None


def test_open_wens_begint_opnieuw_na_een_sluiting():
    """`open_since` moet gewist worden bij dicht, anders veroudert de volgende open mee."""
    state = {}
    flip = datetime(2026, 8, 11, 20, 15)
    wa.track_open_since(state, "office", "open", flip)
    wa.track_open_since(state, "office", "dicht", flip + timedelta(hours=14))
    nieuw = flip + timedelta(hours=24)
    wa.track_open_since(state, "office", "open", nieuw)
    assert wa.open_candidate_age_h(state, "office", nieuw) == 0.0
...
def test_open_bericht_belooft_geen_verzonnen_sluittijd():
    ...
    line = wa.room_message_line("Nursery", d, 22.0, 19.0, [_horizon_interval()])
...
def test_urgent_bericht_benoemt_de_urgentie():
    ...
    line = wa.room_message_line("bedroom", muf, 21.0, 20.0, [], vent_rh=78.0)
    assert "schimmelrisico" in line and "RH ~78%" in line
    heet = {"kind": "dicht", "urgent": True, "reason": None, "urgent_reason": "hitte"}
    assert wa.URGENT_TEXT["hitte"] in wa.room_message_line("Nursery", heet, 21.0, 24.0, [])


def test_advice_message_kop_volgt_de_inhoud():
    op = ("office", {"kind": "open", "urgent": False})
    dicht = ("Nursery", {"kind": "dicht", "urgent": False})
    urgent = ("Nursery", {"kind": "dicht", "urgent": True})
...
def test_build_day_plan_negeert_blips_en_sorteert_op_openingstijd():
    rooms = {
        ...
        "Nursery": {"open_intervals": [_horizon_interval("19:45", 6.5)]},
        "bedroom": {"open_intervals": []},
...
def test_day_plan_message_noemt_openen_en_sluiten_per_kamer():
    plan = [
        {"room": "Nursery", "start_h": -0.25, "windows": [
        ...
        {"room": "bedroom", "start_h": float("inf"), "windows": []},
    ]
    ...
    assert "*Nursery* — staat al open, dicht rond 10:45; daarna open 18:45–21:00" in msg
    ...
    assert "*bedroom* — blijft vandaag dicht" in msg
...
def test_dashboard_gaat_naar_de_artefact_gist(tmp_path, monkeypatch):
    """`window_data.json` draagt per kamer 48u temperatuur én vocht op
    kwartierresolutie — inclusief de badkamer, waar de vochtpieken het
    douche-/aanwezigheidsritme tekenen. Het hoort dus door artefact_io te lopen
    (privé-gist als het secret er is), niet rechtstreeks naar docs/."""
    monkeypatch.setenv("ARTEFACT_GIST_ID", "abc123")
    monkeypatch.setenv("GIST_TOKEN", "tok")
    lokaal = tmp_path / "window_data.json"
    monkeypatch.setattr(wa, "DASHBOARD_FILE", str(lokaal))
    written = {}
    ...
    assert "window_data.json" in written
    # Met het secret gezet blijft er niets in de checkout achter — anders zou de
    # workflow-commit het artefact alsnog publiek in docs/ zetten.
    assert not lokaal.exists()


def test_dashboard_valt_lokaal_terug_zonder_secret(tmp_path, monkeypatch):
    """Zonder ARTEFACT_GIST_ID het oude gedrag (bootstrap/tests), en de read-back
    van `read_prev_dashboard` moet daar dan ook op werken — die levert de
    historie-continuïteit én het om_bias-leerboek."""
    for k in ("ARTEFACT_GIST_ID", "GIST_TOKEN", "GH_TOKEN"):
        monkeypatch.delenv(k, raising=False)
    lokaal = tmp_path / "docs" / "window_data.json"
    monkeypatch.setattr(wa, "DASHBOARD_FILE", str(lokaal))

    wa.write_dashboard({"om_bias": {"night": 1.4}})
    assert wa.read_prev_dashboard() == {"om_bias": {"night": 1.4}}

    monkeypatch.setattr(wa, "DASHBOARD_FILE", str(tmp_path / "weg.json"))
    assert wa.read_prev_dashboard() == {}
