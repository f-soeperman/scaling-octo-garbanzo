...
def test_air_from_sensor_is_de_omkering_van_sensor_temp():
    house = vio.load_house()
    # bedroom heeft sensor_outdoor_frac 0.15; living niet.
    frac = house["rooms"]["bedroom"]["sensor_outdoor_frac"]
    assert frac > 0
    ta, t_out = 24.0, 15.0
    ts = vp._sensor_temp(ta, t_out, frac)
    assert ts != ta                                    # de blend doet écht iets
    assert vp.air_from_sensor(house, "bedroom", ts, t_out) == pytest.approx(ta)
...
def test_air_from_sensor_is_noop_zonder_bias():
    ...
    assert vp.air_from_sensor(house, "bedroom", 22.5, None) == pytest.approx(22.5)
...
def test_anchor_seed_verschuift_de_massaknoop_mee(setup):
    ...
    ta0 = sim["Ta_now"]["nursery"]
    tm0 = sim["Tm_now"]["nursery"]
    meting = ta0 + 1.5                                 # geen sensorbias op nursery → 1-op-1
    seed, tm, n = vfc.anchor_seed(house, sim, {"nursery": meting}, 18.0)
    assert n == 1
    assert seed["nursery"] == pytest.approx(meting)
    # De (Ta − Tm)-differentie codeert of de kamer op- of ontlaadt en blijft behouden.
    assert tm["nursery"] - seed["nursery"] == pytest.approx(tm0 - ta0)
    assert tm["nursery"] == pytest.approx(tm0 + 1.5)


def test_anchor_seed_warm_modus_laat_de_massa_staan(setup):
    house, sim = setup["house"], setup["sim"]
    tm0 = sim["Tm_now"]["nursery"]
    _seed, tm, _n = vfc.anchor_seed(house, sim, {"nursery": sim["Ta_now"]["nursery"] + 1.5},
                                    18.0, tm_mode="warm")
    assert tm["nursery"] == pytest.approx(tm0)             # de naïeve ablatie, bewust bereikbaar


def test_anchor_seed_laat_ongemeten_zones_met_rust(setup):
    house, sim = setup["house"], setup["sim"]
    seed, tm, n = vfc.anchor_seed(house, sim, {"nursery": 25.0}, 18.0)
    assert n == 1
    for z in ("living", "bedroom", "stair", "hall"):
        if z in sim["Ta_now"]:
...
def test_anchor_seed_inverteert_de_sensorbias(setup):
    ...
    house, sim = setup["house"], setup["sim"]
    seed, _tm, _n = vfc.anchor_seed(house, sim, {"bedroom": 24.0}, 15.0)
    assert seed["bedroom"] == pytest.approx(vp.air_from_sensor(house, "bedroom", 24.0, 15.0))
    assert seed["bedroom"] > 24.0                      # buiten kouder → lucht wármer dan de voeler
...
def test_forecast_start_op_de_meting_en_loopt_tot_de_horizon(setup):
    ...
    t0, v0 = fc["series"]["nursery"][0]
    assert t0 == fc["steps"][0]["t"]
    sim_waarde = sim["Ta_now"]["nursery"]
...
def test_latest_actual_negeert_verouderde_metingen():
    vers = {"nursery": [(NOW - timedelta(minutes=20), 21.0), (NOW - timedelta(minutes=5), 22.0)]}
    oud = {"nursery": [(NOW - timedelta(hours=6), 21.0)]}
    assert vfc.latest_actual(vers, NOW) == {"nursery": 22.0}     # nieuwste sample wint
...
def test_driver_export_schema(setup):
    ...
    meta = vfc.driver_export(house, setup["params"], setup["ctx"], fore,
                             ...
                             actual_series={"nursery": [(NOW, 22.4)]})
    ...
    assert set(meta["sensor_rooms"]) == {"living", "nursery", "bedroom", "office", "bath"}
    ...
    assert meta["past"] and meta["actual"]["nursery"]
    ...
    # Alleen echte element-standen: boekhoud-sleutels (`ac_room`/`paused`) en ids die niet
    # (meer) in het huismodel bestaan worden weggefilterd — publiek artefact.
    assert all(isinstance(v, str) for v in step["states"].values())
    huis_ids = vp.reportable_element_ids(setup["house"])
    for s in meta["steps"]:
        assert set(s["states"]) <= huis_ids
        assert not {"ac_room", "paused"} & set(s["states"])
...
def test_driver_export_filtert_boekhoud_sleutels_uit_states(setup):
    """De log accumuleert `ac_room`/`paused` en dode element-ids voorwaarts in elke stap;
    het gepubliceerde vent_forecast.json mag die niet dragen (publiek artefact — de
    ac_room-string overleefde de oude isinstance(str)-filter)."""
    house = setup["house"]
    log = [{"t": (NOW - timedelta(hours=30)).isoformat(),
            "states": {"ac_room": "office", "paused": True, "ted_stair": "open",
                       "nursery_small_window": "open"}}]
    tl = vio.build_timeline(house, _weather_fixture(), log, NOW, 3.0, setup["ctx"],
                            end_h=2.0)
    fore = [s for s in tl if s["t"] >= NOW]
    meta = vfc.driver_export(house, setup["params"], setup["ctx"], fore,
                             setup["sim"]["Ta_now"], setup["sim"]["Tm_now"], now=NOW)
    huis_ids = vp.reportable_element_ids(house)
    assert meta["steps"]
    for s in meta["steps"]:
        assert set(s["states"]) <= huis_ids
        assert not {"ac_room", "paused", "ted_stair"} & set(s["states"])
    # De echte gemelde stand blijft wél staan.
    assert all(s["states"]["nursery_small_window"] == "open" for s in meta["steps"])
