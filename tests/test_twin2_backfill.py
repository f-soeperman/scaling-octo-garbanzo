...
def test_merge_into_shards_dedupes_and_sorts(tmp_path, monkeypatch):
    ...
    shard = json.load(open(tmp_path / "shards" / "2026-06.json"))
    assert shard["rooms"]["Living room"]["ts"] == sorted(shard["rooms"]["Living room"]["ts"])
    # Privacy-scrub: openingen gaan naar het (hier lokale) archief, niet de shard in.
    assert "openings" not in shard
    arch = json.load(open(tmp_path / "archief" / "house_openings_2026-06.json"))
    assert len(arch["log"]) == 1
    # Shard + archief zijn samen direct consumeerbaar door load_dataset.
