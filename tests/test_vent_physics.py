...
def _bedroom_network(wind, p_init=None):
    """Het echte huis met alléén het bedroom-raam open — de configuratie waarop de
    ...
    vol = house["rooms"]["bedroom"]["volume_m3"]
    return net, net["fresh"]["bedroom"] * 3600.0 / vol
...
def test_solve_network_no_discontinuity_across_wind():
    ...
    achs = [_bedroom_network(w)[1] for w in (4.0, 4.5, 5.0, 5.5, 6.0, 6.5, 7.0)]
...
def test_solve_network_result_is_start_independent():
    ...
    net55, ach55 = _bedroom_network(5.5)
    net60, ach60 = _bedroom_network(6.0)
    assert _bedroom_network(6.0, p_init=net55["P"])[1] == pytest.approx(ach60, abs=1e-3)
    assert _bedroom_network(5.5, p_init=net60["P"])[1] == pytest.approx(ach55, abs=1e-3)
...
def test_single_sided_fresh_only_open_vertical_windows():
    ...
    assert vp.single_sided_fresh(house, {"bedroom_window": "dicht"}, {"wind_speed": 2.0},
                                 {"bedroom": 23.0}, 20.0).get("bedroom", 0.0) == 0.0
    ss = vp.single_sided_fresh(house, {"bedroom_window": "open"}, {"wind_speed": 2.0},
                               {"bedroom": 23.0}, 20.0)
    ach = ss["bedroom"] * 3600.0 / house["rooms"]["bedroom"]["volume_m3"]
...
def test_effective_fresh_takes_max_never_sum():
    house = vio.load_house()
    states, weather = {"bedroom_window": "open"}, {"wind_speed": 2.0}
    temps = {"bedroom": 23.0}
    ss = vp.single_sided_fresh(house, states, weather, temps, 20.0)["bedroom"]
    # Netto klein → de eenzijdige term neemt het over (geen som).
    out = vp.effective_fresh({"bedroom": 0.001}, house, states, weather, temps, 20.0)
    assert out["bedroom"] == pytest.approx(ss)
    # Netto groot (echte dwarsventilatie) → het netwerk blijft leidend.
    big = vp.effective_fresh({"bedroom": 10.0}, house, states, weather, temps, 20.0)
    assert big["bedroom"] == pytest.approx(10.0)
...
def test_interzone_conductances_geometry_and_scale():
    house = {"interzone": [{"a": "office", "b": "bedroom", "area_m2": 12, "u": 0.7},
                           {"a": "bedroom", "b": "nursery", "area_m2": 10, "u": 0.5}]}
    g = vp.interzone_conductances(house, {"ua_inter": 1.0})
    assert g[("bedroom", "office")] == pytest.approx(12 * 0.7)   # sleutel alfabetisch geordend
    assert g[("bedroom", "nursery")] == pytest.approx(10 * 0.5)
    # De globale schaal werkt op álle vlakken tegelijk.
    g2 = vp.interzone_conductances(house, {"ua_inter": 2.0})
    assert g2[("bedroom", "office")] == pytest.approx(2 * 12 * 0.7)
...
def test_interzone_conduction_drains_heat_between_stacked_rooms():
    ...
    Dit is de term die de met-de-hoogte-oplopende fout (nursery +0.71 → office +2.19) verklaart."""
    house = _toy_house()
