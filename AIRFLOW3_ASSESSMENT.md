...
vrijloop**. Geen van beide beantwoordt "hoe warm is de kinderkamer morgenochtend om 07:00". Op de
...
| nursery | 0,539 | 1,170 | 0,650 | 0,773 | 1,449 |
| bedroom | 0,912 | 1,243 | 1,731 | 1,804 | 2,392 |
...
exact de grond-gekoppelde kamers te verbeteren (living 55 m², nursery 12, bath 7) en de twee zónder
grondkoppeling (bedroom, office) onaangeroerd te laten. Dat patroon is het argument, niet de
gepoolde score.
...
| bath | 0,420 | **0,363** | **−14 %** |
| nursery | 0,517 | 0,523 | +1 % |
| bedroom | 0,881 | 0,883 | +0 % |
...
zit bij de twee grootste grond-gekoppelde kamers. `nursery` is de uitzondering op de offline
...
kocht living's winst door bedroom van +0,27 naar +0,73 °C te duwen: de aggregatie repareren door
...
1,433 → 1,084) maar verslechterde bedroom (0,883 → **1,468**) en nursery (0,588 → 0,860). Weer
...
**Bekende zwakte, in de UI vermeld:** bij hoge ventilatie kunnen de twee kleinste zones (nursery,
bath) een factor ~2 naast de solver-ACH zitten. `bedroom` en `office` zijn exact, want hun
...
* **Geen achtergrond-infiltratieterm.** Een volledig dicht huis wisselt in dit model géén lucht
  ...
  geabsorbeerd in de geleerde `ua_env`. **Gesloten in §7**: een cónstante infiltratieterm is
  wiskundig dezelfde vorm als `ua_env` op dezelfde knoop en dus na her-seeden per constructie
  onvindbaar; en de gemeten dicht-bias is licht kóud, niet warm.
* **`office.f_air@floor` railt nog steeds** — het overgebleven er-ontbreekt-nog-iets-signaal uit
  de vorige ronde (dak-kamer, zon-naar-massa-split). Onaangeroerd — en bevestigd: in de verse
  5-daagse replay-armen van §7 railt hij opnieuw in béíde armen (de live online-state heeft hem
  vrij op ~0.17, maar elke koude start vanaf de priors drukt hem terug de vloer op).

## 7. De open-raam-plakken — drie kandidaten gemeten, drie keer "nee" (aug 2026)

De scenario-adviezen van het koelplan (Project 14) leunen precies op de raamstand-regimes
waar de minste validatie zat. `tools/horizon_backtest.py --stratify-openings` (nieuw) splitst
de fysica-fout per kamer naar de eigen raamstand op de oorsprong; op het volle record
(~630 oorsprongen, rev 7, live params):

| kamer | dicht | open·wind<2 m/s | open·wind≥2 m/s |
|---|---|---|---|
| living | 0.61 / +0.06 | 0.77 / **+0.43** | 0.77 / **+0.43** |
| nursery | 0.67 / −0.16 | 0.46 / −0.03 | 0.53 / +0.07 |
| bedroom | 0.96 / −0.39 | 1.02 / +0.28 | 0.96 / −0.09 |
| office | 0.72 / −0.15 | 0.97 / −0.18 | 0.78 / +0.05 |
| gepoold | **0.67 / −0.15** | 0.87 / +0.18 | 0.80 / +0.14 |

(RMSE / bias °C. Living's twee open-kolommen zijn tot op 3 decimalen gelijk — nagerekend op de
ruwe dump, écht toeval.) Twee lessen: de dichte-huis-fout is klein en licht **koud** gebiasd,
en het enige structurele open-signaal is **living +0.43 warm, windonafhankelijk** — het model
onderkóélt de woonkamer met open elementen. Dat herschreef de priors van alle drie de
geparkeerde kandidaten:

1. **Achtergrond-infiltratie: gesloten zonder experiment.** Een constante infiltratie-ACH is
   ρ·cp·q·(T_buiten − Ta) op de luchtknoop — exact de vorm van de `ua_env`-term op dezelfde
   knoop met dezelfde driver. Na het verplichte her-seeden van beide armen is het verschil per
   constructie nul: de fit heeft die term al, hij héét alleen ua_env. Alleen een wind-/ΔT-
   afhankelijke infiltratie zou nieuw signaal dragen, en windtermen sneuvelen in dit huis
   structureel op de poorten (bias_eval, §4). Bovendien wijst de gemeten dicht-bias (−0.15,
   licht koud) de verkeerde kant op voor "het dichte huis koelt te traag".
2. **de Gids & Phaff-C-constanten halveren: contra-geïndiceerd door de meting.** De open-stand
   is warm gebiasd — het model ventileert al te wéinig; de constanten halveren verzwakt de
   eenzijdige ventilatie verder. De oorspronkelijke motivatie (vent_eff van zijn vloer halen)
   is bovendien vervallen: vent_eff staat live vrij op ~0.29.
3. **`living_french` als deur i.p.v. klapraam (eff_open_frac 0.5 → 0.9): gemeten, geen effect.**
   Protocol conform de ground rule: beide armen her-geseed met de productie-replay (5 d, 6u-
   stappen, vanaf de priors, zelfde om_bias-driver), daarna de volle backtest per arm.
   Uitkomst: gepoold 0.728 vs 0.729, living 0.737 vs 0.740, living-open-bias +0.39/+0.41 vs
   +0.39/+0.42 — alles binnen ≤0.003 °C, ver onder elke ruisvloer. Bijna een verdubbeling van
   het effectieve openingsoppervlak doet er dus niets toe: bij 0.75 m² effectief is de
   luchtverversing al veel sneller dan de thermische tijdconstante, en de extra m² verzet de
   evenwichtstemperatuur niet. Niet uitgerold.

**Wat het +0.43-signaal dan wél is, blijft open** — met een belangrijke voetnoot: de
raamstand-klassen zijn observationeel, geen experiment. Terrasdeuren staan open op precies de
warmste dagen (dezelfde confound waarom de ML-route destijds is afgewezen), dus een deel van
"open → +0.43" kan gewoon "hittegolf → het model onderschat living's piek" zijn. De plek om
verder te graven is de warme-dag-plak zelf (zon-naar-massa-split, `vent_diagnostics` §3-
residuen per uur), niet de ventilatieterm.

**Operationele bijvangst:** de acceptatiepoort van `tools/vent_seed.py` keurt op het huidige
record ook een ongewijzigde baseline-seed af (`bedroom.c_mass@floor`, `bedroom.solar_gain@floor`
en `office.f_air@floor` railen in de verse replay). Bij de volgende `PHYSICS_REV`-bump gaat
her-seeden dus niet vanzelf — dan eerst het replay-venster of de poort herzien i.p.v. de
afkeuring wegwuiven.
