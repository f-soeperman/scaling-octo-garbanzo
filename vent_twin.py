...
import artefact_io
from notify import read_notify_prefs, run_guarded, sanitize_error, send_telegram
...
from vent_io import (
    ...
    _timedelta_h, ac_changes, ac_room_at, append_forecast_shard, append_history_shard,
    append_shard_weather, apply_routines, build_timeline, collect_actual, collect_heating_on,
    fetch_weather, heating_now, house_location, load_house, load_learned,
    append_openings_archive, load_openings_log, load_window_data, make_context,
    merged_params, migrate_history_shards, migrate_shard_openings,
...
from vent_physics import (
    ...
    build_openings, effective_fresh, reportable_element_ids, per_window_solar, simulate,
    solve_network,
...
def _room_dashboard_row(rid, room, house, params, wd, sim, timeline,
    ...
    return {
        ...
        # Chip: tado-verwarming aan (sensor-readout). De airco- en pauze-boekhouding blijft
        # bewust uit het publieke artefact — de modal leest die live uit de Gist.
...
def build_dashboard(house, params, weather, wd, timeline, sim, learned,
    ...
    # die toont wat écht open/dicht staat i.p.v. de defaults. Gefilterd op de element-ids
    # uit het huismodel: de log accumuleert ook boekhoud-sleutels en hernoemde ids, en
    # die horen niet in het publieke artefact.
    valid_ids = reportable_element_ids(house)
    states_now = {k: v for k, v in openings_at(log, now).items() if k in valid_ids}
    ...
    # `paused` wordt bewust niet (meer) per punt gepubliceerd, en oude punten worden bij het
    # kopiëren ontdaan van het veld: een reeks pauze-punten in een publiek artefact is een
    # machine-leesbare afwezigheidskalender. `held` betekent hier uitsluitend de anomalie-hold
    # (zelf-oplossend binnen ANOMALY_MAX_HOLD_H) — de aanroeper geeft die apart door.
    rmse_hist = [{k: v for k, v in e.items() if k != "paused"}
                 for e in (learned.get("rmse_history") or [])]
    ...
    return {
        ...
        # De pauze- en airco-boekhouding staat bewust níet in dit publieke artefact: de modal
        # leest de actuele stand live uit de Gist (token-houders), en de keuzelijst voor de
        # airco-dropdown komt uit house_meta (sensor-vlag per kamer).
        "rooms": rooms_out,
...
def main():
    ...
    # Airco-kamer uit de kalibratie (geen actieve-koel-term in de fysica). Bewust géén
    # print van kamer of aantallen: de Actions-logs zijn publiek en dit is huisboekhouding.
    acc = ac_changes(log)
    ...
    # Huis-brede pauze: "niemand meldt de standen nu betrouwbaar". Zelfde logstilte als
    # hierboven — de pauzestand mag nooit in een publiek run-log staan.
    ...
    # Stille modus (notify_prefs.json): een stilgezet huis meldt zijn standen per
    # definitie niet betrouwbaar — leren gaat dan in dezelfde stille hold als de
    # huis-pauze, zonder enige publieke marker of afwijkende logregel.
    quiet_now = bool(read_notify_prefs().get("quiet"))
    ...
    if actual:
        ...
        if anomaly_state.get("escaped_at") == now.isoformat():
            ...
            if os.getenv("DRY_RUN") == "1":
                ...
                send_telegram(msg, muted_in_quiet=True)
        # Interne hold (stuurt of calibrate draait) vs. gepubliceerde hold: alleen de
        # anomalie-hold is publiek — die lost zichzelf binnen ANOMALY_MAX_HOLD_H op en
        # verraadt dus geen langdurige afwezigheid; een pauze- of stille-modus-reeks wél,
        # dus die twee holden stil (identieke stdout en een gewoon leercurve-punt).
        published_held = anomaly_held
        learning_held = anomaly_held or paused_now or quiet_now
        if learning_held:
            rmse_now = rmse_cur
            if anomaly_held:
                ...
                # Stille hold (huis-pauze of stille modus): parameters vastgehouden, maar
                # stdout exact als een leer-run — de hold mag nergens publiek af te lezen
                # zijn, óók niet aan een ontbrekende logregel.
    ...
    # overgeslagen — een gepauzeerd/stilgezet huis mag nooit als "geleerd" geboekt worden.
    learning_held = learning_held or paused_now or quiet_now
    ...
    # Vaste dagritmes (house_model.json `routines`, bv. het verduisteringsgordijn dat elke
    # avond dichtgaat): de laatst gemelde stand doortrekken over 12 uur was hier elke nacht
    # aantoonbaar fout. Alleen de vooruitblik — de kalibratie blijft op de gemelde log.
    # De log gaat mee: een expliciete melding in het lopende routinevenster wint.
    fore_tl = apply_routines(fore_tl, house, log=log)
    ...
    dash = build_dashboard(house, params, weather, wd, timeline, sim, learned,
                           ...
                           learning_held=published_held, skill=skill,
                           rmse_baseline=rmse_baseline,
                           heat_now=heat_now,
                           gamma_measured=gamma_measured, wx=wx_summary, forecast=fc)
    # De drie live artefacten gaan naar de privé artefact-gist zodra ARTEFACT_GIST_ID
    # bestaat (privatisering aug 2026, dezelfde gist als data.json/mowing_data.json —
    # het publieke dashboard toonde tot dan de raamstanden zelf, niet alleen of het
    # huis gepauzeerd was). Secret afwezig → het oude lokale pad (bootstrap/tests).
    dash_body = json.dumps(dash, ensure_ascii=False, indent=2)
    dash_dest = artefact_io.write_json_str("vent_data.json", DASHBOARD_FILE, dash_body)
    print(f"[vent_twin] {dash_dest} geschreven ({len(dash_body)} bytes).")
    ...
    if fc["steps"]:
        ...
        fc_body = json.dumps(drivers, ensure_ascii=False)
        fc_dest = artefact_io.write_json_str("vent_forecast.json", FORECAST_FILE, fc_body)
        print(f"[vooruitblik] {fc_dest} geschreven ({len(fc_body) / 1000:.0f} kB).")
    ...
    learned_body = json.dumps(learned_out, ensure_ascii=False, indent=2)
    learned_dest = artefact_io.write_json_str("vent_learned.json", LEARNED_FILE, learned_body)

    # Shards: verse tado-samples + verstreken weer-uren appenden zodat de offline
    # evaluatie-/seed-set blijft aangroeien (merge-op-tijdstip, idempotent). De
    # shards zelf leven sinds de privacy-assessment (aug 2026) in de privé Gist;
    # de openingen-snapshots gingen daar al eerder heen.
    try:
        # Zelf-uitvoerende migratie: nog gecommitte maand-shards → privé Gist,
        # daarna verwijderen (de workflow-commit pikt de deletie op); no-op
        # zodra de boom schoon is. Unie-merge, dus de volgorde t.o.v. de
        # appends hieronder maakt niet uit.
        migrate_history_shards()
        n_s = append_history_shard(wd, log, now, house=house)
        n_w = append_shard_weather(weather, now)
        # Forecast-log: wat Open-Meteo NÚ voor de komende uren voorspelt, zodat de
        # backtest straks ook op échte forecasts kan scoren (--weather forecast).
        n_f = append_forecast_shard(weather, now)
        if n_s or n_w or n_f:
            print(f"[shards] +{n_s} kamersamples, +{n_w} weer-uren, "
                  f"+{n_f} forecast-uren gelogd.")
    except Exception as e:  # noqa: BLE001 — shard-append mag de run nooit laten falen
        print(f"[shards] append overgeslagen: {e}")
    try:
        # Eenmalige zelf-uitvoerende migratie (shard-openings → Gist-archief, daarna
        # strippen; no-op zodra de shards schoon zijn), dan het lopende archief bijwerken.
        # Bewust geen aantallen in het log: elke telling zou een melding dateren.
        migrate_shard_openings()
        append_openings_archive(log)
        print("[archief] openingen-archief bijgewerkt.")
    except Exception as e:  # noqa: BLE001 — archief-bijwerking mag de run nooit laten falen
        print(f"[archief] bijwerken overgeslagen: {sanitize_error(e)}")

    print(f"[vent_twin] Geschreven: {dash_dest} + {learned_dest}. Klaar.")
