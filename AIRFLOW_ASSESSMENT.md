...
een kunstmatige zelfde-gevel-lus van ~27 ACH die warme buitenlucht in bedroom blies (+3.0 °C
...
| bedroom | **+2.98** | **+0.96** | **1.11** | structureel te warm, hele dag — zie §4.2 |
...
("de thermische voorspelling is er ondanks dit goed") bleek achterhaald: met bedroom- én
office-raam open rekende het netwerk een 0.24 m³/s-lus (27 ACH) ín bedroom, dóór de koker, úít
...
0.43, en de tweeling blies intussen 24°-buitenlucht in bedroom (gemeten 21.7°, voorspeld 24.7°).
...
×0.5). Gemeten op het echte huis: lus 0.24 → 0.02 m³/s, bedroom-ACH 27 → ~3, windongevoelig
...
**4.2 bedroom is de structureel slechtste kamer** (+0.96 gem, +3.0 nu): grotendeels de valse
instroom-lus van §3.3 (fix moet dit dempen — hét validatiepunt van de komende dagen), maar
bedroom was ook vóór de lus de meest rail-verzadigde kamer (solar_gain, h_am, q_int én f_air
op de vloer). Restverdenking: de geometrie-placeholders (volume 32 m³ en muur 12 m² identiek
aan nursery/office — zelf-gelabeld "SCHATTINGEN") en de buitenmuur-voeler (`sensor_outdoor_frac`
...
horizon-masks (de boom bij de kinderkamer is een erkende grove benadering), overstek/terras-schaduw —
...
2. **Fysica-rev 2: `WIND_REF_Z` + `EFF_OPEN_AREA`** — verwacht na her-convergentie (uren, door
   de globalen-reset + checkpoint-vervaltijd): bedroom-fout zakt substantieel; ACH-waarden
...
4. **Geometrie-huiswerk** (meten, geen code): echte volumes/muuroppervlakken voor
   nursery/bedroom/office (nu identieke placeholders), en de voeler-fracties.
