...
Fetches hourly forecast from Open-Meteo and sends a briefing covering a set of
time blocks plus daily UV exposure windows.

The blocks themselves are a privacy boundary and live in the BRIEFING_BLOCKS
secret, not in this file — see the comment at DEFAULT_BLOCKS below. Without the
secret the briefing runs on generic parts of the day.

The location is fixed (Utrecht). Temporarily silencing the briefing goes via
the quiet mode (notify_prefs.json — see notify.py), never via a location edit.
"""
...
# De vaste thuislocatie. Bewust GEEN hand-aanpasbare "waar ik nú ben"-variant meer:
# een afwijkende locatie zetten betekende een publieke commit met de bestemming erin,
# plus een afwijkende berichtkop in het publieke run-log — precies het soort spoor dat
# hier nooit mag ontstaan. Wie de briefing tijdelijk niet wil, gebruikt de stille modus
# (notify_prefs.json, zie notify.py) — die laat niets publieks achter.
LOCATION = {
...
# ── Tijdblokken — PRIVACY-GRENS (privacy-sweep aug 2026) ─────────────────────────
#
# De blokken vormen samen een weekkalender van wanneer het huishouden weg is en
# wanneer het thuiskomt. Dat is precies het "indicator dat ik weg ben"-signaal
# waar de banner bovenaan CLAUDE.md over gaat — en het stond hier letterlijk in
# de publieke broncode én gepubliceerd op het inmiddels verwijderde iPad-
# dashboard. De echte blokken horen daarom in een secret, net als de tado-
# zonenamen (`TADO_ZONE_ALIASES`). Beschrijf ze hier ook niet in prozawoorden:
# een comment is net zo publiek als de constante.
#
# env BRIEFING_BLOCKS = JSON:
#   {"weekday":     [["label", sh, sm, eh, em, [dagen]|null, "icoon"], ...],
#    "weekend":     [[...], ...],
#    "weekend_days": [4, 5, 6]}
# dagen: 0=maandag … 6=zondag; null = elke dag van die set.
#
# Afwezig/onparseerbaar → de generieke dagdelen hieronder. De briefing blijft dan
# gewoon werken (ochtend/middag/avond), alleen niet op maat. Dat is bewust het
# publieke gedrag: wie de repo leest, leert het ritme van dit huishouden niet.
DEFAULT_BLOCKS = [
    ("Ochtend",  7,  0, 10,  0, None, "🌅"),
    ("Middag",  12,  0, 15,  0, None, "🌤"),
    ("Avond",   18,  0, 21,  0, None, "🌙"),
]
DEFAULT_WEEKEND_DAYS = {5, 6}   # zaterdag, zondag


def _parse_blocks(raw: list) -> list[tuple]:
    """Eén blokkenlijst uit de secret-JSON → de interne tuple-vorm.

    Een enkel misvormd blok laat de rest staan; de aanroeper valt pas terug op de
    generieke dagdelen als er niets bruikbaars overblijft."""
    out = []
    for b in raw or []:
        try:
            label, sh, sm, eh, em, days, icon = b
            out.append((str(label), int(sh), int(sm), int(eh), int(em),
                        set(days) if days is not None else None, str(icon)))
        except (TypeError, ValueError):
            continue
    return out


def load_blocks() -> tuple[list, list, set]:
    """(weekdag-blokken, weekend-blokken, weekend-dagen) uit `BRIEFING_BLOCKS`.

    Zonder het secret: de generieke dagdelen voor beide sets. Nooit raisen — een
    kapotte secret mag het weerbericht niet stilleggen."""
    raw = os.getenv("BRIEFING_BLOCKS", "")
    if not raw:
        return DEFAULT_BLOCKS, DEFAULT_BLOCKS, DEFAULT_WEEKEND_DAYS
    try:
        cfg = json.loads(raw)
        week = _parse_blocks(cfg.get("weekday")) or DEFAULT_BLOCKS
        weekend = _parse_blocks(cfg.get("weekend")) or week
        days = cfg.get("weekend_days")
        return week, weekend, set(days) if days else DEFAULT_WEEKEND_DAYS
    except (json.JSONDecodeError, AttributeError, TypeError):
        print("[briefing] BRIEFING_BLOCKS onparseerbaar — generieke dagdelen", file=sys.stderr)
        return DEFAULT_BLOCKS, DEFAULT_BLOCKS, DEFAULT_WEEKEND_DAYS
...
def build_message(location, forecast, today, pollen=None):
    ...
    week_blocks, weekend_blocks, weekend_days = load_blocks()
    blocks = weekend_blocks if weekday in weekend_days else week_blocks
...
def main():
    ...
    pollen   = fetch_pollen(LOCATION)
    message  = build_message(LOCATION, forecast, today, pollen=pollen)

    # De berichttekst hoort niet in het publieke Actions-log (dagroutine + weerdetails);
    # alleen een handmatige DRY_RUN-dispatch mag 'm printen om te debuggen.
    ...
    send_telegram(message, chat_id=os.getenv("TELEGRAM_CHAT_GROUP_ID"),
                  parse_mode="Markdown", muted_in_quiet=True)
