...
**Goal:** Doorlopend (jaarrond, 4×/dag) overzicht van waar en wanneer **vier steden** — Utrecht (thuisbasis), Berlijn, Parijs en Madrid — geschikt zijn om te kamperen met tent, peuter en auto: dagen < 30°, nachten > 10° én ruim boven het dauwpunt (droge tent), dagregen zwaarder gewogen dan nachtregen, officiële waarschuwingen van **oranje of hoger** als rode vlag — **geel wordt bewust volledig genegeerd** (bewonersbesluit aug 2026: een gele hittegolfwaarschuwing kleurde vrijwel de hele matrix rood) — en vertrekken na een droge nacht + ochtend. **Dashboard-only:** geen Telegram-advies, alleen de gedeelde `run_guarded`-crash-alert.

### De stedenwissel (sep 2026) — de inhoud wisselt, de cadans nooit
Dit project is dashboard-only en draait onveranderd 4×/dag. Dat is een
**privacy-eigenschap, geen implementatiedetail**: de runs en de commits van
`camping_data.json` staan publiek in de Actions-lijst, dus elke ingreep op de
*cadans* — een uitgezette cron, een `if: false`, een handmatig disabled
workflow — publiceert een gedateerd "vanaf hier niet meer". Op een publieke
repo bestaat een onzichtbare pauze niet; een stille pijplijn ís het signaal.
Wil je dus iets aan dit project veranderen, verander dan **wát het berekent**,
nooit hoe vaak het draait. Dat is bij de stedenwissel ook gebeurd: de
regiolijst ging van twaalf uitgekamde Oost-Franse plaatsen naar vier
wereldsteden (Utrecht, Berlijn, Parijs, Madrid) — generieker dan wat er stond,
en workflow, artefactvorm en scoring bleven ongemoeid.
De kampeercriteria (tent, peuter, dauwpunt) zijn **bewust niet** meegewisseld:
dat een hoofdstad geen kampeerweide is, is de grap — geen reden om `PEN`,
`MIN_NIGHTS` of `MOVE_PENALTY` te hertunen.
...
- `docs/js/campingmap.js` + `tools/campingmap_build.py` — de overzichtskaart: een **gecommit build-artefact** (vereenvoudigde Natural Earth-landcontouren + de vaste pin-coördinaat per stad), géén run-artefact. De bouwtool draagt een **mirror van `REGIONS`** en een bounding box; bij een wijziging van de regiolijst moeten allebei mee en moet `python3 tools/campingmap_build.py` opnieuw draaien (netwerk nodig, ~3 MB GeoJSON). Bij de stedenwissel moest de box wél mee: Madrid lag onder de oude ondergrens en zou buiten de viewBox vallen. Een pin zonder data in het artefact wordt grijs getekend, dus de kaart overleeft het venster tussen een push en de eerstvolgende run

### Per-run flow
1. MeteoAlarm-feeds per land (NL/DE/FR/ES sinds de stedenwissel; `COUNTRY_FEEDS` in `meteoalarm.py` kent ook AT) — niet-fataal: uitval → `warnings_status[land]="failed"` (het artefact onderscheidt "geen waarschuwingen" altijd van "feed onbereikbaar", het dashboard toont er een banner voor).
...
Een tegel onder de matrix beantwoordt de vraag *"waar zitten we mét
verplaatsen het beste?"*: `SUPER_REGIONS` partitioneert de niet-Utrecht-regio's
— sinds de stedenwissel (sep 2026) één groep, **Stedentrip**
(berlijn/parijs/madrid) — en een test bewaakt dat de partitie exact en
disjunct dekt. Daarvóór waren het er twee (Noordoost- en Oost-Frankrijk), en
daarvóór weer Oostenrijk + Noordwest-Frankrijk; de partitie-test is bij elke
wissel het enige dat aangepast hoefde te worden. **Let op bij het lezen van de
route:** `MOVE_PENALTY` is geijkt op tent-afbreken-met-een-peuter, niet op
1000 km tussen twee hoofdsteden, dus verkassen is nu te goedkoop en de route
hupt eerder dan je in het echt zou doen — bewust niet hertuned (domeinbesluit),
maar wel benoemd in de dashboard-legenda. `besteSuper` zet géén "★ beste
keuze" meer bij één enkele tegel (dat is geen keuze).
Per super-regio rekent `flex_route` (klein dynamisch
...
- **Opentijd-meter** (`meter_step`): reconstrueert per kraan de voltooide open-intervallen uit opeenvolgende snapshots (activity + timestamp + duration, altijd geklemd op [vorige check, nu]; onzin-timestamps degraderen naar de vorige check, nooit naar garbage). **Dicht-op-dicht verzint nooit een beurt** (aug 2026, bewonersbesluit na een gemelde vals-positieve registratie): tot dan werd een verschoven activity-ts tussen twee dicht-metingen gelezen als bewijs van een volledig binnen het gat geopende-en-gesloten beurt (halve-venster-schatting) — maar die timestamp bleek ook zonder een echte open/dicht-cyclus te kunnen verschuiven, en leverde zo een geregistreerde beurt op voor een kraan die (bevestigd via de GARDENA-app-historie) nooit was opengegaan. Twee dicht-metingen op rij leveren nu onvoorwaardelijk géén interval op, ook niet als de ts verschoof — bewuste ondertelling, zelfde afweging als de sluittijd-terugval hieronder. **De geregistreerde mm komen altijd uit de meter**, ook voor eigen sessies — eerlijker dan commando-rekensommen bij vroegtijdige stops.
