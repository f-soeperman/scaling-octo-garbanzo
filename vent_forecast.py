...
def _driver_step(house: dict, zones: list[str], ctx: RunContext, s: dict) -> dict:
    ...
    **Niets hier wordt afgerond.** Elk getal in deze dict gaat rechtstreeks het RC-stelsel in,
    ...
    return {
        ...
        # Alleen echte element-standen uit het huismodel: de log accumuleert ook
        # boekhoud-sleutels (`ac_room`/`paused`) en hernoemde ids, en dit artefact is
        # publiek — de JS-kern leest toch alleen de element-kolommen.
        "states": {k: v for k, v in s["states"].items()
                   if isinstance(v, str) and k in vp.reportable_element_ids(house)},
    }
