...
# De kruipruimte onder living/nursery is die koude put. Hij is geventileerd — dus deels gekoppeld
...
# nursery −19 %, bath −13 %; bedroom 0.863 → 0.870, office 0.712 → 0.711). Een alternatief dat
# gepoold béter scoorde (`INTERNAL_NIGHT_FRACTION` 1.8) is juist daarom verworpen: dat kocht
# living's winst door bedroom van +0.27 naar +0.73 te duwen — de aggregatie repareren door de
...
# luchtadvectie door open deuren. In werkelijkheid zijn nursery → bedroom → office op elkaar
...
# hoogte — precies het gemeten patroon (nursery +0.71, bedroom +1.32, office +2.19 °C op een
...
# deurhoogte door de gekoppelde kamers (nursery 1.0m, bedroom 3.9m, office 7.0m) — die kamers grenzen
...
# dan nursery bij open deur, het spiegelbeeld van de 28.4°-float die deze term juist moest fixen).
...
def param_bounds(house: dict | None, scope: str, name: str) -> tuple[float, float]:
    ...
    Een GLOBALE `c_mass`-vloer is getest en verworpen: die hielp living maar duwde bedroom'
...
def facade_irradiance(facade_az: float, sun_az: float, sun_el: float,
    ...
    massaknoop-warmbias van juli 2026, de kinderkamer). De boom-hoogte zelf is een seizoens-/
...
# bedroom-raam → koker → kantoor-raam van ~0.27 m³/s bij 3 m/s (~27 ACH; 0.57 bij 6.2 m/s — de
...
# blies intussen warme buitenlucht in bedroom (de +3°C-fout van 10 juli). Motor van de fix: de
...
def _shade_factor(wid: str, w: dict, states: dict) -> float:
    ...
    - **simpel scherm/gordijn** (`factor`): open ×1.0, dicht ×factor, half ertussenin —
      vaste opaciteit, je trekt 'm dicht of open (b.v. het verduisteringsgordijn van de kinderkamer).
...
def solve_network(zones: list[str], openings: list[dict], zone_temps: dict[str, float],
    ...
    if not converged:
        ...
        # sprong de druk van bedroom heen en weer tussen ~12.5 en ~0.4 Pa, terwijl de
...
def reportable_element_ids(house: dict) -> set[str]:
    """Alle element-ids die een gerapporteerde stand kúnnen dragen volgens het huismodel:
    ramen, roosters, deuren en de `<raam>_shade`-zonweringslaag. De openingen-log
    accumuleert voorwaarts en houdt dus ook sleutels vast die niet (meer) in het
    huismodel bestaan — boekhoud-sleutels en hernoemde element-ids. Publicatiepaden
    (dashboard-`openings`, forecast-`states`) filteren daarop zodat alleen de echte,
    actuele elementen het publieke artefact bereiken."""
    ids = set(house.get("windows", {})) | set(house.get("vents", {})) | set(house.get("doors", {}))
    ids |= {wid + "_shade" for wid, w in house.get("windows", {}).items() if w.get("shade")}
    return ids
...
def air_from_sensor(house: dict, rid: str, t_sensor: float | None,
    ...
    `sensor_outdoor_frac` 0.15 (bedroom, office), en valt op als een grote fout op h=1 —
...
def _interp(series: list[tuple], ts: datetime) -> float:
    ...
    for (t0, v0), (t1, v1) in zip(series, series[1:]):
        if t0 <= ts <= t1:
            # span <= 0 (dubbele samples na een dubbele dispatch) → v0, geen
            # verzonnen 1s-span die f naar 0 klapt (zelfde guard als
            # window_advisor._interp_out_corr).
