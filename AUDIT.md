# Code Quality Audit — Pineapple Under The Sea
...
| R8 | Low | `airflow_model.py:1524-1618` | `calibrate` guards NaN per GN step, but the final blended `rmse` is unguarded until a later `rmse_now == rmse_now` check; params are not clamped to `BOUNDS` inside the solver (rails only reported). | **Fixed** (verified juli 2026) — final-blend NaN guard returns the old params; `_clamp_to_bounds` clamps every GN iterate in-solver |
