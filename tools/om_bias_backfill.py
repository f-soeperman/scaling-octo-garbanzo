...
def main() -> int:
    ...
    # Sinds de privacy-sweep (aug 2026) leeft het artefact in de privé artefact-gist;
    # met ARTEFACT_GIST_ID + GIST_TOKEN in de omgeving lezen/schrijven we dáár, anders
    # het lokale docs/-pad (bootstrap/tests). Zelfde bestand, andere bewaarplaats.
    path = os.path.join(args.repo, WINDOW_DATA_PATH)
    wd = artefact_io.read_json("window_data.json", path, default=None)
    if not isinstance(wd, dict):
        print(f"FOUT: window_data.json niet leesbaar (gist noch {WINDOW_DATA_PATH}).")
        return 1
    ...
    body = json.dumps(wd, indent=2, ensure_ascii=False)
    dest = artefact_io.write_json_str("window_data.json", path, body)
    print(f"[backfill] om_bias-blok geschreven → {dest} ({len(body) / 1024:.0f} kB).")
    return 0
