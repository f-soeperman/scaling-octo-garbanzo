...
experimenteren — zonder de repo-fysica. Sinds de shard-privatisering
(privacy-assessment aug 2026) leven de maand-shards in de privé Gist:
draai met `GIST_ID`/`GIST_TOKEN` in de env (read-only), of wijs
`VENT_HISTORY_DIR` naar een lokale shard-kopie voor een volledig offline run
(de shards bevatten het weer).
...
# `paused` staat bewust NIET (meer) in de export: de dataset gaat als publiek
# downloadbaar Actions-artefact de deur uit en een pauze-kolom is een machine-
# leesbare afwezigheidskalender. De pauze-vensters worden intern nog wél gebruikt
# om samples te filteren (filter_paused_samples), alleen de kolom vervalt.
HOUSE_STATE_COLS = ["ac_room", "ac_here"]
...
def wide_columns(room_ids, element_ids, baseline: bool):
    ...
    cols = SHARED_META + WEATHER_COLS + ["ac_room"] \
        + [f"open_{e}" for e in element_ids]
    for rid in room_ids:
...
def to_wide(long_rows, room_ids, element_ids, baseline: bool):
    ...
    shared_keys = SHARED_META + WEATHER_COLS + ["ac_room"] \
        + [f"open_{e}" for e in element_ids]
    by_t = {}
    for r in long_rows:
...
def build_schema(room_ids, element_ids, baseline: bool, elem_meta: dict):
    ...
    return {
        ...
        "notes": [
            "Gebouwd uit de twin2-maand-shards (privé Gist sinds aug 2026, of een lokale kopie via VENT_HISTORY_DIR).",
