...
appendde. LET OP sinds de shard-privatisering (aug 2026): met GIST_ID/GIST_TOKEN
in de env leest én ververst dit de shards in de privé Gist — draai de verversing
niet terwijl de kwartierloop actief is (die is de reguliere enige schrijver), of
zet VENT_HISTORY_DIR naar een lokale kopie. Schrijft docs/vent_learned.json (VENT_LEARNED_PATH) met een additieve
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    # Zelfde artefact-gist als de productierunner (ARTEFACT_GIST_ID); dit schrijft
    # bewust de PRODUCTIE-vent_learned.json — bij een PHYSICS_REV-bump gaat dit
    # tooltje echt live, dus geen apart lokaal-only pad hier.
    body = json.dumps(out, ensure_ascii=False, indent=2)
    dest = artefact_io.write_json_str("vent_learned.json", vio.LEARNED_FILE, body)
    print(f"[seed] geschreven: {dest} (seed_src: {out['seed_src']})")
