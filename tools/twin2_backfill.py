...
Reconstrueert de trainingsset (maand-shards; sinds de privacy-assessment aug 2026
in de privé Gist als `twin2_history_<YYYY-MM>.json` — mét creds en zonder `--out`
schrijft dit script dus rechtstreeks de Gist; draai het NIET terwijl de
kwartierloop actief is, die is de reguliere enige schrijver) uit drie bronnen:
...
def extract_room_samples(wd: dict, skip: set[str] | None = None) -> dict[str, dict[int, tuple]]:
    """{kamernaam: {epoch: (temp, hum, heat)}} uit één window_data.json-versie.

    `skip` = de kamers met `exclude_from_shards` (privacy-sweep aug 2026); zonder
    dit zou een re-mint uit de git-historie het douche-spoor terugzetten dat de
    runner sinds die sweep juist weglaat."""
    skip = skip or set()
...
def merge_into_shards(samples: dict[str, dict[int, tuple]], openings: list[dict]) -> int:
    """Voeg de gedolven samples toe aan de maand-shards en de gedolven openingen aan
    het privé Gist-archief (dedupe op tijdstip; idempotent). Openingen gaan sinds de
    privacy-scrub (aug 2026) bewust NIET meer de gecommitte shards in — zet
    VENT_OPENINGS_ARCHIVE_DIR of GIST_ID/GIST_TOKEN voor de archiefkant. Geeft het
    aantal nieuw toegevoegde samples."""
    ...
    n_arch = vio.append_openings_archive(openings)
    if openings and not n_arch:
        print("[backfill] let op: openingen niet gearchiveerd (geen archief bereikbaar "
              "of alles al aanwezig).")
    return added
...
def main() -> int:
    ...
    skip = vio.shard_excluded_rooms(vio.load_house())
    if skip:
        print(f"[backfill] uitgesloten van de shards (exclude_from_shards): {sorted(skip)}")
    for i, (sha, _dt) in enumerate(commits):
        wd = git_show_json(sha, WINDOW_DATA_PATH, args.repo)
        if not wd:
            continue
        merge_samples(samples, extract_room_samples(wd, skip))
        if (i + 1) % 10 == 0:
